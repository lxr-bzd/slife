...
public class SysDictService extends BaseService<SysDictDao, SysDict> implements ISysDictService {
    ...
    public List<JsTree> getDictTree() {
        ...
        return list(new QueryWrapper<SysDict>().lambda().orderByAsc(SysDict::getSort)).stream().map(dict -> {
    ...
    public PCAjaxVO delete(Long id) {
        ...
        if (sysDict != null) {
            //删除
            remove(new QueryWrapper<SysDict>().lambda().likeRight(SysDict::getPath, sysDict.getPath()));
        } else {
